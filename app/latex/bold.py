...
Also escapes bare LaTeX special characters in plain AI output.
Run this AFTER the AI returns, BEFORE injecting into the template.
"""

import re

# Tech terms from Ayush's resume stack. Sorted longest-first so multi-word
# matches (e.g. "Gemini Flash 2.5") take priority over partial matches.
TECH_TERMS: set[str] = {
    # AI / ML
    ...
    # Other
    "Pandas", "NumPy", "SciPy", "Gmail", "Outlook", "GPU", "Agile", "Scrum",
}
...
def apply_bold(plain_text: str) -> str:
    ...
    # Bold percentages: 95% → \textbf{95\%}
    text = _PCT_RE.sub(r"\\textbf{\1\\%}", text)

    # Bold numbers/metrics: 128K, 2.5s, 500+
    ...
    # Bold tech terms (longest match first)
    for term in _SORTED_TERMS:
        if term in text:
            text = text.replace(term, f"\\textbf{{{term}}}")

    # Escape remaining bare LaTeX special chars
